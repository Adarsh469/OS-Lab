/**
 * MRU Page Replacement (Exam variation)
 * Replace the page with HIGHEST last-use time
 * Compile: gcc mru.c -o mru
 */
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (found) {
            ...
            if (empty != -1) {
                ...
                int mru = 0;
                for (int j = 1; j < frames_count; j++) if (time[j] > time[mru]) mru = j;  // HIGHEST
                frames[mru] = pages[i];
                time[mru] = counter++;
            }
            faults++;

/**
 * LRU using 8-bit Reference Registers
 * Shift right, set MSB=1 on reference, replace smallest value.
 * Compile: gcc ref_register_lru.c -o ref_register_lru
 */
...
int main() {
    ...
    printf("\n========== LRU WITH 8-BIT REFERENCE REGISTERS ==========\n");
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (found) {
            ...
            if (empty != -1) {
                ...
                int lru = 0;
                for (int j = 1; j < frames_count; j++) if (reg[j] < reg[lru]) lru = j;
                frames[lru] = pages[i];
                reg[lru] = 0x80;
            }
            faults++;
